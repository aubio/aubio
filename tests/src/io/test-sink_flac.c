...
// this file uses the unstable aubio api to test aubio_sink_flac, please
...
int main (int argc, char **argv)
{
  ...
  aubio_sink_flac_t *o = new_aubio_sink_flac(sink_path, 0);
  if (!o) { err = 1; goto beach_sink; }
  err = aubio_sink_flac_preset_samplerate(o, samplerate);
  if (err) { goto beach; }
  err = aubio_sink_flac_preset_channels(o, channels);
  if (err) { goto beach; }

  do {
    aubio_source_do_multi(i, mat, &read);
    aubio_sink_flac_do_multi(o, mat, read);
    n_frames += read;
  } while ( read == hop_size );
  ...
  PRINT_MSG("wrote %s with %dHz in %d channels\n", sink_path,
      aubio_sink_flac_get_samplerate(o),
      aubio_sink_flac_get_channels(o) );

beach:
  del_aubio_sink_flac(o);
beach_sink:
  del_fmat(mat);
beach_fmat:
  del_aubio_source(i);
...
#else /* HAVE_FLAC */
  err = 0;
  PRINT_ERR("aubio was not compiled with aubio_sink_flac\n");
#endif /* HAVE_FLAC */
  return err;
}
